...
export default function RiskAssessmentPage() {
  ...
  useEffect(() => {
    ...
    if (user.roles && user.roles.some((role) => role.slug === "compliance")) {
  ...
  return (
    <div className="min-h-screen w-full overflow-x-hidden bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100">
      ...
      <div className="container mx-auto px-2 xs:px-3 sm:px-4 md:px-1 lg:px-2 py-6 xs:py-8 sm:py-10 md:py-7 lg:py-1 w-full max-w-md xs:max-w-lg sm:max-w-2xl md:max-w-4xl lg:max-w-5xl xl:max-w-6xl 2xl:max-w-7xl">
        {/* Header */}
        <motion.div
              ...
              >
                ...
                >
                  {(criteria[currentStep - 1]?.options ?? []).map(
                    (option, index) => {
                      const isSelected =
                        responses[criteria[currentStep - 1].id] === option.id;
                      return (
                        <motion.div
                          key={option.id}
                          initial={{ opacity: 0, y: 20 }}
        ...
        {/* Navigation */}
        <div className="flex flex-col sm:flex-row justify-between items-center mt-6 sm:mt-8 w-full max-w-full sm:max-w-2xl mx-auto gap-4 sm:gap-0 px-2">
          <Button
            variant="outline"
            onClick={handlePrev}
